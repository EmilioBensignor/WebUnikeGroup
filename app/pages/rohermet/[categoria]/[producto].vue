  ...
  <DefaultMain>
    ...
    <DefaultSection class="py-6 px-4 md:p-16 md:pb-8 lg:pt-44 lg:pb-12 lg:px-16 xxl:py-32">
      <div class="w-full xxl:max-w-[1304px] flex flex-col lg:flex-row gap-6 md:gap-8 lg:gap-12 xxl:gap-16">
        <RohermetProductoGaleria class="hidden lg:block xxl:w-[40%]"
          :imagen-principal="producto?.imagen_principal" :galeria="producto?.galeria" :alt="producto?.nombre" />
        <div class="lg:w-1/2 xxl:w-[60%] flex flex-col gap-6 xxl:gap-8">
          <div class="flex flex-col md:flex-row-reverse gap-6">
            <div class="md:w-1/2 lg:w-full flex flex-col md:justify-center gap-3 md:gap-4 lg:gap-6 xxl:gap-8">
              <div class="flex flex-col gap-1 lg:gap-3">
                ...
                <p v-if="producto?.capacidad_lts" class="lg:text-[1.75rem] text-terciary font-semibold">{{
                  producto?.capacidad_lts }} lts</p>
              </div>
              <p class="text-sm lg:text-md text-terciary font-medium whitespace-pre-line">{{ producto?.descripcion }}
              </p>
              <RohermetProductoBotones v-if="producto" :producto="producto" class="hidden md:flex lg:hidden" />
            </div>
            <RohermetProductoGaleria class="lg:hidden"
              :imagen-principal="producto?.imagen_principal" :galeria="producto?.galeria" :alt="producto?.nombre" />
          </div>
          ...
          <div class="flex flex-col gap-6 xxl:gap-8">
            <div>
              <div v-if="productosRelacionados.length > 1" class="grid grid-cols-4 items-end">
                <NuxtLink v-for="(relacionado, index) in productosRelacionados" :key="relacionado.id"
                  :to="`${ROUTES_NAMES.ROHERMET.HOME}/${relacionado.categoria?.slug}/${relacionado.slug}`"
            ...
            <RohermetProductoBotones v-if="producto" :producto="producto"
...
<script setup>
definePageMeta({ layout: 'rohermet' })
...
const {
  ...
  fetchImagenesRedes,
  getProductoImageUrl
} = useRohermetProductos()

const { setSeoForProducto } = useRohermetSeo()

const producto = ref(null)
const productosRelacionados = ref([])
const imagenesRedes = ref([])

const categoriaColor = '#497A9B'
...
const getOpcionIcon = (opcion) => {
  ...
  if (normalizado.includes('interior')) return '/images/waterplast/producto/exterior.svg'
  return '/images/waterplast/producto/dimensiones.svg'
}

const getRelacionadoStyle = (relacionado, index) => {
  const isSelected = relacionado.slug === route.params.producto
  const baseColor = categoriaColor
...
const mapProductoUrls = (data) => ({
  ...data,
  imagen_principal: data.imagen_principal ? getProductoImageUrl(data.imagen_principal) : (data.imagen ? getProductoImageUrl(data.imagen) : null),
  imagen: data.imagen ? getProductoImageUrl(data.imagen) : null,
  ficha_tecnica: data.ficha_tecnica ? getProductoImageUrl(data.ficha_tecnica) : null,
  manual_instalacion: data.manual_instalacion ? getProductoImageUrl(data.manual_instalacion) : null,
...
const fetchProducto = async () => {
  try {
    ...
    const productoConUrls = mapProductoUrls(data)

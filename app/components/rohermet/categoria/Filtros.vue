        ...
        <div
                    ...
                    <Transition enter-active-class="transition-all duration-300 ease-out md:transition-none"
                        ...
                        <div v-if="filtrosAbiertos || esPantallaGrande" class="flex flex-col gap-6 text-terciary">
                            ...
                            <div class="flex flex-col gap-4">
                                ...
                                <!-- Uso -->
                                <div v-if="tieneUsoFiltro" class="flex flex-col gap-1.5">
                                    <h3 class="text-sm lg:text-base font-bold">Uso</h3>
                                    <div class="flex flex-col gap-1.5">
                                        <label v-for="uso in usosDisponibles" :key="uso"
                                            class="flex items-center justify-between cursor-pointer group">
                                            <span
                                                class="text-xs lg:text-sm text-terciary font-medium group-hover:text-secondary transition-colors">{{
                                                    uso
                                            ...
                                            <div class="relative inline-flex items-center">
                                                <input :id="`uso-${uso}`" :name="`uso-${uso}`"
                                                    v-model="filtros.uso[uso]" type="checkbox"
                                                    class="sr-only peer" />
                                                <div
                                                    class="w-12 h-7 bg-gray-mid peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full peer-checked:after:border-white after:content-[''] after:absolute after:top-1 after:left-1 after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all after:duration-300 peer-checked:bg-secondary transition-colors duration-300">
                                                </div>
            ...
            <div class="w-full flex flex-col gap-4 px-4 pb-6 md:p-0">
                ...
                <div v-else class="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-2 gap-6 md:gap-2 lg:gap-x-6 lg:gap-y-4">
                    <RohermetCategoriaCard v-for="producto in productosVisibles" :key="producto.id"
                        :producto="producto" />
                </div>
...
const usosDisponibles = computed(() => {
    if (!props.productos?.length) return []

    const usosArr = []
    const tieneInterior = props.productos.some(p => p.opcion === 'Para interiores')
    const tieneExterior = props.productos.some(p => p.opcion === 'Para exteriores')

    if (tieneInterior) usosArr.push('Interior')
    if (tieneExterior) usosArr.push('Exterior')

    return usosArr
})

const tieneUsoFiltro = computed(() => usosDisponibles.value.length > 1)
...
const filtrosAplicados = computed(() => {
    ...
    Object.keys(filtros.value.uso).forEach(uso => {
        if (filtros.value.uso[uso]) {
            aplicados.push(`Uso ${uso}`)
        }
    })
...
const productosFiltrados = computed(() => {
    ...
    return props.productos.filter(producto => {
        ...
        const usosSeleccionados = Object.keys(filtros.value.uso).filter(uso => filtros.value.uso[uso])
        if (usosSeleccionados.length > 0) {
            if (!producto.opcion) return false
            const cumpleUso = usosSeleccionados.some(uso => {
                return producto.opcion === usoOpciones[uso]
            })
            if (!cumpleUso) return false
        }
...
const removerFiltro = (index) => {
    ...
    else if (filtros.value.orientacion.hasOwnProperty(filtro)) {
        filtros.value.orientacion[filtro] = false
    }
    else if (filtros.value.color.hasOwnProperty(filtro)) {
        filtros.value.color[filtro] = false
    }
    else if (filtros.value.tecnologia.hasOwnProperty(filtro)) {
        filtros.value.tecnologia[filtro] = false
    }
    else if (filtro.startsWith('Uso ')) {
        const uso = filtro.replace('Uso ', '')
        if (filtros.value.uso.hasOwnProperty(uso)) {
            filtros.value.uso[uso] = false
        }
    }
}
</script>

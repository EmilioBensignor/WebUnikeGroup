    ...
    <div class="w-full md:w-1/2">
        ...
        <div v-else-if="!producto" class="text-center py-8">
            ...
            <p class="text-sm text-dark mt-2">
                Marca: {{ marca }} | Categoría: {{ route.params.categoria }} | Producto: {{ route.params.producto }}
            </p>
        </div>
...
const { processProductoHTML } = marca.value === 'waterplast' ? waterplastComposable : rohermetComposable
...
const get3DViewerUrl = () => {
    ...
    if (producto.value.archivo_html) {
        ...
        return `${config.public.supabase.url}/storage/v1/object/public/${marca.value}-productos/${producto.value.archivo_html}`
    }
...
const getImageUrl = (imagen) => {
    ...
    return `${config.public.supabase.url}/storage/v1/object/public/${marca.value}-productos/${imagen}`
}

const fetchProducto = async () => {
    try {
        const { data, error } = await supabase
            .from(`${marca.value}-productos`)
            .select(`
                *,
